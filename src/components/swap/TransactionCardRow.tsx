...
export function TransactionCardRow({
  ...
  return (
    <Link target="_blank" href={`https://sepolia.etherscan.io/tx/${txHash}`}>
      <Card
      ...
      >
        ...
        <span className="flex-1 text-sm">{`${sender.slice(0, 6)}...${sender
          .slice(-4)
          .toLocaleLowerCase()}`}</span>

        <span className="flex-1 text-sm">{dateRelative(String(dateTime))}</span>
